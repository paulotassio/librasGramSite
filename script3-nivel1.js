...
function drop(ev) {
    ...
    if (target.classList.contains('target')) {
        if (target.dataset.fase.trim() != null) {
            ...
            var requiredWords = ["tampa", "caneta", "quebrada"];
            var allWordsPresent = requiredWords.every(word => correctWords.includes(word));

            if (contador == 3 && allWordsPresent) {
                var fraseCompleta = "a tampa da caneta está quebrada"; 
...
function avancar() {
    /*alert("PARABÉNS!");*/
    window.location.href = "nivel4n1.html?pontuacao=" + pontuacao2;
}
